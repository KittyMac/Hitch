...
int i, v, n;
	...
	for (i = 0; i < n; i ++) {
		v  = downcase (b0->data[i])
		   - downcase (b1->data[i]);
		if (0 != v) return v;
	}

	if (b0->slen > n) {
		v = downcase (b0->data[n]);
		if (v) return v;
		return UCHAR_MAX + 1;
	}
	if (b1->slen > n) {
		v = - downcase (b1->data[n]);
		if (v) return v;
...
int i, v, m;
	...
	if (b0->slen > m) {
		v = downcase (b0->data[m]);
		if (v) return v;
		return UCHAR_MAX + 1;
	}

	v = - downcase (b1->data[m]);
	if (v) return v;
	return - (int) (UCHAR_MAX + 1);
}
...
int i;
	...
	for (i=0; i < len; i++) {
		if (b->data[i] != ((unsigned char*)blk)[i]) {
			unsigned char c = downcase (b->data[i]);
			if (c != downcase (((unsigned char*)blk)[i]))
				return 0;
		}
	}
	return 1;
...
inline int blkequalblk (unsigned char * b0, int len0, unsigned char * b1, int len1) {
    if (len0 < 0 || len1 < 0) return BSTR_ERR;
